...
The system SHALL extract candidate records from listing pages via `extract_listing_candidates()` which: (1) parses content blocks (`article`, `.card`, `.views-row`, etc.) for titles, summaries, and links, (2) parses anchor elements directly, (3) filters candidates through `is_funding_opportunity_heuristics()`, (4) deduplicates by canonical URL. Fallback: if no candidates found, captures all PDF links as records.

#### Scenario: Block-based extraction
- **WHEN** a listing page contains `<article>` elements with `<a>` links
- **THEN** each article becomes a candidate with title, summary, detail_url, and pdf_url
