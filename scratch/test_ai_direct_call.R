...
cfg <- get_ai_config()
message(sprintf("Provedor: %s | Modelo: %s", cfg$provider, cfg$model))
...
prompt <- paste(
  ...
  "",
  "Texto do Edital:", "O objetivo da FINEP com este edital é financiar o desenvolvimento de foguetes no Brasil. A FINEP vai apoiar projetos com valor total de fomento de R$ 5.000.000,00. A seleção será feita em lotes de 12 empresas de cada vez."
)
...
if (!is.null(res)) {
  parsed <- try(jsonlite::fromJSON(res, simplifyVector = TRUE), silent = TRUE)
  message("\n--- PARSED ---")
  print(parsed)
}
